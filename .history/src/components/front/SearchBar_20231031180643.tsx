        ...
        <CommandList>
            {
                data?<>{
                    data?.map((p)=>{
                        return(<>
                          <CommandItem><div className='flex justify-between w-full items-center'><div className='flex items-center'><Image className='rounded mr-5 w-10 h-10' src={p.images[0].url} alt={'product image'} width={30} height={30}  /> {p.name}</div> <div className='flex items-center'><div>{p.price} TND </div><div className='flex items-center'><Button>view</Button></div></div></div></CommandItem>
                        </>)
                    })

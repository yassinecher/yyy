        ...
        <CommandList>
            {
                data?<>{
                    data?.map((p)=>{
                        return(<>
                          <CommandItem><div className='flex justify-between'><div className='flex'><Image className='rounded mr-5' src={p.images[0].url} alt={'product image'} width={30} height={30}  /> {p.name}</div></div></CommandItem>
                        </>)
                    })

...
const Header: React.FC<HeaderProps> = ({ session, cathegories, noscategy, links }) => {
  ...
  return (
    <>  <>
      ...
      <div className={` ${isMenuOpen ? 'flex' : 'hidden'} text-md fixed top-0 left-0 w-full h-full flex-col dark:bg-black bg-[#fff7edf5] opacity-100 z-50`}>
        ...
        <Accordion type="single" collapsible>
          {links.map((linkk) => (<>
            {linkk.link.length > 0 ? <>
              <NavigationMenuItem className="px-3 list-none bg-orange-100 rounded-sm text-center border-1 border-orange-300 m-2 py-2 dark:text-black ">

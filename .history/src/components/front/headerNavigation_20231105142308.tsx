                  ...
                  noscategy.map((item,key)=>(<>
                   ...
                   onMouseMoveCapture={()=>{setCatstodisplay(noscategy[key].CathegoryCollectiondata.map((item)=>item.Label));setSelected(key)}}
                    onClick={()=>{setCatstodisplay(noscategy[key].CathegoryCollectiondata.map((item)=>item.Label));setSelected(key)}}
                     className={`${selected==key? 'bg-amber-500 dark:bg-amber-50 dark:text-black text-white ':" bg-transparent text-black"}px-7  hover:bg-white hover:text-black dark:hover:bg-amber-100 dark:hover:text-black py-9 rounded-none justify-start text-left rounded-none"`}>{item.Label}</Button>

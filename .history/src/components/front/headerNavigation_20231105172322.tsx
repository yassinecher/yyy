                  ...
                  noscategy.map((item,key)=>(<>
                   ...
                   onMouseMoveCapture={()=>{setCatstodisplay(noscategy[key].CathegoryCollectiondata.map((item)=>item.Label));setSelected(key)}}
                    onClick={()=>{setCatstodisplay(noscategy[key].CathegoryCollectiondata.map((item)=>item.Label));setSelected(key)}}
                       className={` px-7 
                      mt-2 py-9 justify-start hover:bg-amber-100  hover:rounded-l-md hover:rounded-r-none text-left  "${selected==key? ' mr-0 dark:bg-amber-50 dark:text-black text-black rounded-none rounded-l-md bg-amber-50  ':" border-1 border-amber-100 bg-white text-black mr-2 rounded-md"}`}>{item.Label}</Button>

...
const Header: React.FC<HeaderProps> = ({ session, cathegories, noscategy, links }) => {
  ...
  return (
    <>  <>
      ...
      <div className={` ${isMenuOpen ? 'flex' : 'hidden'} text-md fixed left-0 w-full h-screen bottom-0  flex-col dark:bg-black bg-[#fffffff5] opacity-100 z-50`}>
          ...
          {links.map((linkk) => (<>
            {linkk.link.length > 0 ? <>
              ...
              <AccordionItem className='px-3' value={linkk.id}>
                ...
                <Button
                  className="mx-14 list-none bg-orange-50 text-black w-full bg-opacity-50 rounded-sm text-center border-1 border-orange-100 m-2 py-2 dark:text-black " >

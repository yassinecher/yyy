...
const Header: React.FC<HeaderProps> = ({ session, cathegories }) => {
  ...
  return (
    <>  <>
    {session?.user.role === "admin" ? (  <Link href={"/admin"} passHref>
        <button className='bg-red-700 w-full text-white '>Admin Panel</button></Link>) : <></>}
   </>
    


      <div className="dark:bg-black opacity-90 border-b-2 border-b-slate-800 bg-white sticky top-0 z-50" style={backgroundStyle}>

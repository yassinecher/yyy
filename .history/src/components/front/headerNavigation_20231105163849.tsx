        ...
        <NavigationMenuItem>
          ...
          <NavigationMenuContent className="  ">
            <ul className="  dark:border-black  grid gap-3 md:w-[400px] rounded-none lg:w-[800px]  lg:grid-cols-3 ">
              <li className=" p-3 pt-0  pr-0 dark:text-white dark:bg-gray-800 row-span-1 h-full flex flex-col bg-white ">
                 {
                  noscategy.map((item,key)=>(<>
                   ...
                   onMouseMoveCapture={()=>{setCatstodisplay(noscategy[key].CathegoryCollectiondata.map((item)=>item.Label));setSelected(key)}}
                    onClick={()=>{setCatstodisplay(noscategy[key].CathegoryCollectiondata.map((item)=>item.Label));setSelected(key)}}
                       className={`${selected==key? 'bg-amber-50   mr-0 dark:bg-amber-50 dark:text-black text-black rounded-l-md':" border-1 border-amber-100 bg-white text-black mr-2 rounded-md"} px-7 
                      mt-2 py-9 justify-start hover:bg-amber-100  hover:rounded-l-md hover:rounded-r-none text-left "`}>{item.Label}</Button>

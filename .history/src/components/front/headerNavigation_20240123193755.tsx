 ...
 const HeaderNavigation: React.FC<HeaderProps>=({noscategy ,cathegories,links})=> {
  ...
  <NavigationMenu.Root className="NavigationMenuRoot">
    ...
    {links.map((linkk ,key)=>(<>
      ...
      <NavigationMenu.Item>
          <NavigationMenu.Trigger   className="focus:bg-transparent text-black dark:bg-transparent dark:text-black bg-transparent rounded-none hover:bg-transparent active:bg-transparent NavigationMenuTrigger" onMouseMoveCapture={()=>{setCatstodisplay([]);setSelected(NaN)}}  >{linkk.label}
          </NavigationMenu.Trigger>
          <NavigationMenu.Content className="NavigationMenuContent">
            <ul className={`"${catstodisplay.length>0?' grid lg:grid-cols-3 lg:[800px]':' grid lg:grid-cols-1 lg:[300px] h-full'} custom-nav  dark:border-black   gap-0 md:[400px] rounded-none   "`}>
              <li className=" p-3  pt-0  pr-0 dark:text-white dark:bg-gray-950 rospan-1 h-full flex flex-col bg-white ">
                 {

...

                    {
                        data ? <CommandGroup heading="Results">

                            {
                                data?.map((p) => {
                                    return (<>
                                        <CommandItem onClick={()=>{router.push(`/product/${p.id}`);setOpen(false)}} value={p.name} ><div onClick={()=>{router.push(`/product/${p.id}`);setOpen(false)}} className='flex justify-between w-full items-center'>
                                            <div onClick={()=>{router.push(`/product/${p.id}`);setOpen(false)}} className='flex items-center'>
                                                ...
                                                {p.name}</div> <div className='flex items-center'><div>{p.price} TND </div><div className='flex items-center ml-5'>
                                                          ...
                                                          <IconButton onClick={()=>{router.push(`/product/${p.id}`);setOpen(false)}}  className=' bg-white bg-opacity-50'

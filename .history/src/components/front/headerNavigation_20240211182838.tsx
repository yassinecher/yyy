      ...
      <NavigationMenu.Item>
          ...
          <NavigationMenu.Content className=" NavigationMenuContent ">
            <ul className={` w-max dark:border-black   gap-0   ${catstodisplay.length>0?' grid lg:grid-cols-3 custom-nav lg:w-[800px]':' grid lg:grid-cols-1  h-full'} `}>
              <li className=" p-3  pt-0  pr-0 dark:text-white dark:bg-purple-950 row-span-1 h-full flex flex-col bg-white ">
                 {
                  ...
                  {item.navitemId=== linkk.id?<>
                   ...
                   onMouseMoveCapture={()=>{setCatstodisplay(noscategy[key].CathegoryCollectiondata.map((item)=>item.Label));setSelected(key)}}
                    onClick={()=>{setCatstodisplay(noscategy[key].CathegoryCollectiondata.map((item)=>item.Label));setSelected(key)}}
                       className={` px-7 
                      mt-2 py-9 justify-start hover:bg-purple-100  hover:rounded-l-md hover:rounded-r-none text-left  "${selected==key? ' mr-0 dark:bg-purple-800 dark:text-white text-black rounded-none rounded-l-md bg-purple-50  ':" border-1 dark:border-0 dark:text-white border-purple-100 bg-white dark:bg-purple-900 text-black mr-2 rounded-md"}`}>{item.Label}</Button>
...
>(({ className, title, children, ...props }, forwardedRef) => {
  return (
    <li>
       <NavigationMenu.Link asChild className="bg-white dark:bg-purple-900 bg-opacity-50  dark:bg-opacity-50 border text-center hover:bg-opacity-100">

...
const Invoice = (props: { invoiceData: CartItem[] ,order:Order|null}) => {
  ...
  const generatePDF = () => {
    const pdfOptions = {
      ...
      image: { type: 'pdf', quality: 1.0 }, // Set quality to 1.0 for maximum quality
      html2canvas: { scale: 2, useCORS: true }, // Adjust scale and enable useCORS for better quality
      jsPDF: { unit: 'mm', format: 'a4', orientation: 'portrait' },
    };
    const invoiceElement = document.getElementById('your-invoice-element-id');

  if (invoiceElement) {
    const existingClasses = invoiceElement.className;

    // Remove dark theme class (if it exists) and add day theme class
    invoiceElement.classList.replace('dark','ddd');
    html2pdf().from(invoiceElement).set(pdfOptions).save('invoice.pdf').outputPdf();
    invoiceElement.className = existingClasses;
  } else {
  ...
  return (
    <div  className='container bg-white dark:bg-slate-950 py-5 rounded-lg my-5'>
      <div ref={invoiceRef} id='your-invoice-element-id' className='bg-white dark:bg-slate-950 p-3'  >   <h1 className='text-3xl font-bold text-amber-500 dark:text-amber-400'>Facture</h1>

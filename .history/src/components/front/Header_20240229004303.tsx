...
const Header: React.FC<HeaderProps> = ({ session, cathegories, noscategy, links,noscategyMobile ,linkMobile}) => {
  ...
  return (
    <> 
          ...
          <div className="  items-center align-middle  h-[56px] hidden sm:flex justify-center">
            ...
            <div className="w-full bg-white shadow-md shadow-zinc-500  drop-shadow-sm  flex justify-center h-14 ">
              <div className='m-auto flex justify-center'>
                ...
                <Button className='ml-5 dark:bg-[#673AB7] dark:text-white bg-[#673AB7] hover:bg-[#673AB7] focus:hover:bg-[#673AB7]' variant={'default'} >
                  Build your pc
                </Button> </Link>
              </div>
      ...
      <div className={` ${isMenuOpen ? 'flex' : 'hidden'} text-md fixed left-0 w-full min-h-screen top-0  flex-col dark:bg-[#121212f5] bg-[#fffffff5] opacity-100 z-50`}>
        <div className=' overflow-y-auto overscroll-x-none overflow-x-clip px-3 w-full'>
        ...
        <Link href={"build-pc"} className='p-5  w-full max-w-full' onClick={()=>setIsMenuOpen(!isMenuOpen)}>
               <div>
               <Button className='w-full dark:bg-[#673AB7] dark:text-white bg-[#673AB7]' variant={'default'} >
                  Build your pc
                </Button>
               </div>
                </Link>
          ...
          {linkMobile.map((linkk) => (<>
            {linkk.link.length > 0 ? <>
              ...
              <AccordionItem className='px-3' value={linkk.id}>
                ...
                <AccordionContent>
                  ...
                  {item.navitemId=== linkk.id?<>
                    <AccordionItem value={item.id}>
                      <AccordionTrigger   className='active:text-[#673AB7] focus:text-[#673AB7]    px-10 '>{item.Label}</AccordionTrigger>
                ...
                <Button
                  className="m-2 mx-2 list-none  dark:border-0 dark:bg-[#262626ab] dark:text-white bg-[#b4a2d4] text-black w-full bg-opacity-50 rounded-sm text-center border-1 border-[#9a86bd]  py-2 " >

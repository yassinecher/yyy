...
const Invoice = (props: { invoiceData: CartItem[] ,order:Order|null}) => {
  ...
  return (
    <div className='container bg-white dark:bg-slate-950 py-5 rounded-lg my-5'>
      <h1 className='text-3xl font-bold text-amber-500 dark:text-amber-400'>Facture</h1>
      {order?<>
        <div>
        <div>Vendeur:</div>
      <div className='flex items-center font-bold text-lg'>  <Image className='mr-3' alt='logo' width={50} height={50} src={'/images/logo (3).png'}/> Gaming Gear TN</div>
        ...
        <div>Client:</div>
         <div className='flex items-center font-bold text-lg'>nom & prénom :{order.name} {order.lastName}</div>
      <div className='flex items-center font-bold text-lg'>Adresse E-mail{order.email}</div>

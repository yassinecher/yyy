        ...
        <CommandList>
            {
                data?<>{
                    data?.map((p)=>{
                        return(<>
                          <CommandItem><div className='flex justify-between w-full'><div className='flex'><Image className='rounded mr-0' src={p.images[0].url} alt={'product image'} width={30} height={30}  /> {p.name}</div> <div><div>{p.price} TND </div><div><Button>view</Button></div></div></div></CommandItem>
                        </>)
                    })

        ...
        <NavigationMenuItem>
          ...
          <NavigationMenuContent className="  ">
            <ul className="  dark:border-black  grid gap-3 md:w-[400px] rounded-none lg:w-[800px]  lg:grid-cols-3 ">
              <li className=" p-3 pt-0  dark:text-white dark:bg-gray-800 row-span-1 h-full flex flex-col bg-amber-100 ">
                 {
                  noscategy.map((item,key)=>(<>
                   ...
                   onMouseMoveCapture={()=>{setCatstodisplay(noscategy[key].CathegoryCollectiondata.map((item)=>item.Label));setSelected(key)}}
                    onClick={()=>{setCatstodisplay(noscategy[key].CathegoryCollectiondata.map((item)=>item.Label));setSelected(key)}}
                       className={`${selected==key? 'bg-amber-100 dark:bg-amber-50 dark:text-black text-black ':" bg-amber-200 text-black"}px-7 
                       hover:bg-white pt-3 hover:text-black dark:hover:bg-amber-100 dark:hover:text-black py-9 rounded-none justify-start text-left rounded-none"`}>{item.Label}</Button>

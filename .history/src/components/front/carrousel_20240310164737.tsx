            ...
            style={{
              ...
              backgroundPosition: "center", // You can adjust this property based on your needs
                ...
                <div className='basis-2/3 p-28 dark:text-white max-sm:p-0 max-sm:flex max-sm:align-middle max-sm:content-center max-sm:flex-col '>
                    ...
                    {
                      each.title.length>0?<>
                       <h1 className={`p-3 font-black tracking-wider text-2xl slideTitle  bg-opacity-30 ` }

style={{color:` ${each.titleColor.length>0?`${each.titleColor}`:''}`}}
...
>{each.title}</h1>
  ...
  <p className='py-7' style={{ color: each.descriptionColor.length > 0 ? each.descriptionColor : '' }}>
    {each.description}
  </p>
) : null}

{each.DeletedPrice.length > 0 ? (
  <p className='py-2 line-through' style={{ color: each.DeletedPriceColor.length > 0 ? each.DeletedPriceColor : 'text-gray-400 dark:text-gray-400' }}>
    {each.DeletedPrice} TND
  </p>
) : null}
{each.Price.length > 0 ? (
  <p className='py-2' style={{ color: each.PriceColor.length > 0 ? each.PriceColor : '' }}>
    {each.Price} TND
  </p>
) : null} 

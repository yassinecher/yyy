...
const Header: React.FC<HeaderProps> = ({ session, cathegories, noscategy, links }) => {
  ...
  return (
    <>  <>
      ...
      <div className={` ${isMenuOpen ? 'flex' : 'hidden'} text-md fixed top-0 left-0 w-full h-full flex-col dark:bg-black bg-[#fffffff5] opacity-100 z-50`}>
        ...
        <Accordion type="single" collapsible>
          {links.map((linkk) => (<>
            {linkk.link.length > 0 ? <>
              <NavigationMenuItem className="px-3 list-none bg-orange-50 bg-opacity-50 rounded-sm text-center border-1 border-orange-100 m-2 py-2 dark:text-black ">
                ...
                <AccordionContent>
                  ...
                  {item.navitemId=== linkk.id?<>
                    <AccordionItem value={item.id}>
                      <AccordionTrigger className='px-10 '>{item.Label}</AccordionTrigger>
                      <AccordionContent>
                        {
                          noscategy[key].CathegoryCollectiondata.map((i)=>(<>
                           <NavigationMenuItem className="mx-14 list-none bg-orange-50 bg-opacity-50 rounded-sm text-center border-1 border-orange-100 m-2 py-2 dark:text-black ">
                         <Link href={`/shop?categorie=${i.Label}`}>
                  {i.Label}
                </Link>
              </NavigationMenuItem>

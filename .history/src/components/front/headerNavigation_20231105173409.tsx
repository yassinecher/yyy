  ...
  return (
    <NavigationMenu>
      <NavigationMenuList>
        ...
        <NavigationMenuItem>
          <NavigationMenuTrigger  className="focus:bg-transparent text-black dark:bg-transparent dark:text-black bg-transparent rounded-none hover:bg-transparent active:bg-transparent ">TOUTES NOS CATÉGORIES
          </NavigationMenuTrigger>
          <NavigationMenuContent className="  ">
            <ul className="  dark:border-black  grid gap-3 md:w-[400px] rounded-none lg:w-[800px]  lg:grid-cols-3 ">
              <li className=" p-3 pr-0 pt-0  pr-0 dark:text-white dark:bg-gray-950 row-span-1 h-full flex flex-col bg-white ">
                 {
                  noscategy.map((item,key)=>(<>
                   ...
                   onMouseMoveCapture={()=>{setCatstodisplay(noscategy[key].CathegoryCollectiondata.map((item)=>item.Label));setSelected(key)}}
                    onClick={()=>{setCatstodisplay(noscategy[key].CathegoryCollectiondata.map((item)=>item.Label));setSelected(key)}}
                       className={` px-7 
                      mt-2 py-9 justify-start hover:bg-amber-100  hover:rounded-l-md hover:rounded-r-none text-left  "${selected==key? ' mr-0 dark:bg-gray-900 dark:text-white text-black rounded-none rounded-l-md bg-amber-50  ':" border-1 border-amber-100 bg-white text-black mr-2 rounded-md"}`}>{item.Label}</Button>
                  
                  </>))
                 }
              </li>
              <div className="col-span-2 grid grid-cols-2 p-5 bg-white bg-opacity-75 dark:text-white ">
                {

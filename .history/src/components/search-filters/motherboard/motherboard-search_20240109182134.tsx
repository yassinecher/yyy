...
const SearchComponent: React.FC<SearchComponentProps>  = ({setProducts,setLoading}) => {
  ...
  const handleSearch = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    const filtered1 = filtered.filter((item) => item.price >= new Decimal (priceFilter[0])&&item.price <= new Decimal(priceFilter[1]));
  ...
  const handlePriceFilter = (e: number[]) => {
    ...
    const filtered = data.filter((item) => item.price >= new Decimal(priceFilter[0]) &&item.price <= new Decimal(priceFilter[1])) ;

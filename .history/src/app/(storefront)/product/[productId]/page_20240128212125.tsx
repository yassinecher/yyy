...
if(prod.Laptop.length>0){
  ...
  if(prodet.Graphiccard){
    data.push({id:prodet.Graphiccard?.id??i.toString(),name:"Carte gr",value:prodet.Graphiccard.name??'zz'})
    i++
  }
  if(prodet.Hardisk){
    data.push({id:prodet.Hardisk?.id??i.toString(),name:"",value:prodet.Hardisk.name??'zz'})
    i++
  }

 
  if(prodet.RefreshRate){
    data.push({id:prodet.RefreshRate?.id??i.toString(),name:"",value:prodet.RefreshRate.name??'zz'})
    i++
  }
  if(prodet.ScreenSize){
    data.push({id:prodet.ScreenSize?.id??i.toString(),name:"",value:prodet.ScreenSize.name??'zz'})
    i++
  }
  if(prodet.ScreenType){
    data.push({id:prodet.ScreenType?.id??i.toString(),name:"",value:prodet.ScreenType.name??'zz'})
    i++
  }
  if(prodet.System){
    data.push({id:prodet.System?.id??i.toString(),name:"",value:prodet.System.name??'zz'})
    i++
  }
  if(prodet.TouchScreen){
    data.push({id:i.toString(),name:"",value:prodet.TouchScreen?"oui":'non'})
    i++
  }
  if(prodet.memory){
    data.push({id:prodet.memory?.id??i.toString(),name:"",value:prodet.memory.name??'zz'})
    i++
  }
  if(prodet.network){
    data.push({id:prodet.network?.id??i.toString(),name:"",value:prodet.network.name??'zz'})
    i++
  }
  if(prodet.Sound){

...
const CustomPackTemplate: React.FC<ProductFormProps> = ({ initialData}) => {
    ...
    const [keyboardList, setkeyboardList] = useState<ProdCol[]>(initialData && initialData.PackProduct && initialData.PackProduct[0].Clavier
      ...
      useEffect(() => {
        ...
        // Add prices of selected default products to the total
        if (defaultKeyboard) {
            total += Number(defaultKeyboard.price);
        }
        if (defaultMouse) {
            total += Number(defaultMouse.price);
        }
        if (defaultMousePad) {
            total += Number(defaultMousePad.price);
        }
        if (defaultMics) {
            total += Number(defaultMics.price);
        }
        if (defaultHeadset) {
            total += Number(defaultHeadset.price);
        }
        if (defaultCamera) {
            total += Number(defaultCamera.price);
        }
        if (defaultScreen) {
            total += Number(defaultScreen.price);
        }
        if (DefaultSpeaker) {
            total += Number(DefaultSpeaker.price);
        }
        if (DefaultManette) {
            total += Number(DefaultManette.price);
        }
        if (DefaultChair) {
            total += Number(DefaultChair.price);
        }
        total-=Number(initialData?.PackProduct[0].discountOnPack)
    ...
    const onAddToCart = () => {
        var stockArray: Product[]=[]
            ...
            const pc: CartItem = {
                ...
                defaultMousePad:defaultMousePad as unknown as Product,
                defaultMics:defaultMics as unknown as Product,
                defaultHeadset:defaultHeadset as unknown as Product,
                defaultCamera:defaultCamera as unknown as Product,
                defaultScreen:defaultScreen as unknown as Product,
                DefaultSpeaker:DefaultSpeaker as unknown as Product,
                DefaultManette:DefaultManette as unknown as Product,
                DefaultChair:DefaultChair as unknown as Product,
                number: 1
            }
            cart.addItem(pc)

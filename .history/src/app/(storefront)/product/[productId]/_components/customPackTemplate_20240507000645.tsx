...
const CustomPackTemplate: React.FC<ProductFormProps> = ({ initialData}) => {
    ...
    const [keyboardList, setkeyboardList] = useState<ProdCol[]>(initialData && initialData.PackProduct && initialData.PackProduct[0].Clavier
      ...
      useEffect(() => {
        ...
        if (defaultKeyboard) {
            total += defaultKeyboard.price;
        }
        if (defaultMouse) {
            total += defaultMouse.price;
        }
        if (defaultMousePad) {
            total += defaultMousePad.price;
        }
        if (defaultMics) {
            total += defaultMics.price;
        }
        if (defaultHeadset) {
            total += defaultHeadset.price;
        }
        if (defaultCamera) {
            total += defaultCamera.price;
        }
        if (defaultScreen) {
            total += defaultScreen.price;
        }
        if (DefaultSpeaker) {
            total += DefaultSpeaker.price;
        }
        if (DefaultManette) {
            total += DefaultManette.price;
        }
        if (DefaultChair) {
            total += DefaultChair.price;
        }

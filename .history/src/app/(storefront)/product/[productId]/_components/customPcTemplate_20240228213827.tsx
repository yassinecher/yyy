...
const CustomPcTemplate : React.FC<ProductFormProps>= ({initialData,motherboards,gpus,cpus,cases,powersupplies,cooling,diks,rams}) => {
    ...
    useEffect(() => {
      ...
      list=[
        ...list,
        {
          composant_name:simplifyMemoryConfiguration(ram),
          composant_Type: "Ram",
          composant_Destails: "",
  ...
  function simplifyMemoryConfiguration(configuration: string): string {
    const modules: RegExpMatchArray | null = configuration.match(/\((.*?)\)/g);
    if (!modules) return "Invalid configuration";

    const moduleCounts: { [key: string]: number } = {};

    modules.forEach(module => {
        const typeMatch: RegExpMatchArray | null = module.match(/DDR\d/);
        if (typeMatch) {
            const type: string = typeMatch[0];
            moduleCounts[type] = (moduleCounts[type] || 0) + 1;
        }
    });

    const simplifiedConfig: string = Object.entries(moduleCounts)
        .map(([type, count]) => `${count} (${type} X ${type === 'DDR4' ? '8 GB' : '16 GB'})`)
        .join(' , ');

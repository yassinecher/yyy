...
export const Motherboard = (props: {
  ...
  const fetchData = async () => {
    try {
      ...
      const response = await fetch(`/api/motherboard/component?minDt=${priceFilter[0]}&maxDt=${priceFilter[1]}&q=${searchTerm}&sort=${selectedSort}&units=10&page=${currentPage}&filterList=${encodedFilterList}${compatible && props.motherboardId ? `&cpuId=${props.motherboardId.id}` : ''}`);

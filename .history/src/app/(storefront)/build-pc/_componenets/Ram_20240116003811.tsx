...
const Number = async () => {
  ...
  const updateElementAtIndex = (ind: number, newValue: Memory | undefined) => {
    const index = ind
    ...
    if (newValue === undefined) {
      ...
      if (is2caseProduct(newValue)) {
        ...
        if (props.rams.filter((e) => e === undefined).length >= 2) {
          ...
          const indexToRemove = props.rams.findIndex((item) => item === undefined);
          if (indexToRemove !== -1) {
            props.setRams((prevRams: (Memory | undefined)[]) => {
              const updatedArray = [...prevRams];
              updatedArray.splice(indexToRemove-1, 1);
              return updatedArray;
            });
          }
        } else {

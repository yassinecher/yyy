    ...
    const [openedramSlot, setopenedramSlot] = useState(0)
     ...
     const addEmptyCase = () => {
      ...
      const removeEmptyCase = () => {
        const indexToRemove = props.rams.findLastIndex((item) => item === undefined);
        if (indexToRemove !== -1) {
           props.setRams((prevRams: (Memory | undefined)[]) => {
...
const Number = async () => {
  ...
  const updateElementAtIndex = (ind: number, newVe: Memory | undefined) => {
    ...
    if (newValue === undefined) {
      ...
      if (is2caseProduct(newValue)) {
        ...
        if (props.rams.filter((e) => e === undefined).length >= 2) {
            ...
            toast.error('Ce updatewithremove2case a 2 barrettes RAM, veuillez libérer un autre emplacement RAM.1');
        } else {
    ...
    useEffect(() => {
        ...
        if (ramSlotNumber === 2) {
          // Initialize props.rams with two undefined values
          ...
          for(let Xy=array1.length;Xy<2;Xy++){
           ...
           props.setRams(()=>{
            ...
            for(let Xy=array1.length;Xy<4;Xy++){
              array1[Xy]=undefined
            }
             props.setRams(()=>{
                return [...array1]});
        }
      }, [ramSlotNumber, props.rams, is2caseProduct, updateElementAtIndex]);

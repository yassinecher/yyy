...
type YourComponentProps = {
    ...
    setAllProductCompatibility:(value:AllProductsCompatibility)=>void,
    memoryMarque: Filter
  ...
  const updateElementAtIndex = (ind: number, newVe: Memory | null) => {
    ...
    if (newValue === null) {
      ...
      if (is2caseProduct(newValue)) {
        ...
        if(
            prvprod && is2caseProduct(prvprod)&&!is2caseProduct(newValue)
        ){ 
            updateonecasefortwocase(index,prvprod)
    ...
    function extractLesserNumber(memoryString: string): number | null {
        const memoryStringWithoutSpaces = memoryString.replace(/\s/g, '').toUpperCase(); // Remove all spaces
    ...
    const ramCompatibility=(mem:Memory,index:number)=>{
        ...
        if(mem!=null){
            if(mem.memories[0].type.name===ramSlotType){
                message='Barrette RAM Compatible'
                comp=true
            }else{
                message='Barrette RAM '+ramSlotType+' est requis'
                comp=false
            }
        }
        return {
                                ...
                                <button onClick={() => steOpenDialog(true)} className='lg:w-1/5 w-full min-w-md:max-w-lg  m-3 bg-transparent border-transparent hover:bg-transparent hover:border-transparent '>
                                    <Card className="   ">
                                        <CardHeader>
                                            ...
                                            <p className='text-xs text-[#f59e0b] p-1'>(*Obligatoire)</p></CardTitle>
                                        </CardHeader>
                                        <CardContent>
			...
			c0.445,1.724,2.013,3,3.874,3s3.428-1.276,3.874-3h14.031C44.009,2,44.5,2.491,44.5,3.096V11z"/>
                                ...
                                <Card className="lg:w-4/5  w-full min-w-md:max-w-lg justify-center flex align-middle  lg:flex-row flex-col items-center m-3">
                                    <CardContent className='p-0 w-full h-full'>
                                        {
                                            props.rams[0] != null ? <>       <div className="dark:bg-[#000000e6] bg-[#ffffffe6] flex  justify-between group   md:flex-row flex-col  items-center rounded-xl space-x-3">
                                                {/* Image & actions */}
                                        ...
                                        <button onClick={() => steOpenDialog(true)} className='lg:w-1/5 w-full min-w-md:max-w-lg  m-3 bg-transparent border-transparent hover:bg-transparent hover:border-transparent '>
                                            <Card className="   ">
                                                <CardHeader>
                                                    ...
                                                    {props.rams.findIndex((e)=>e!==null)!=-1?<></>:<><p className='text-xs text-[#f59e0b] p-1'>(*Obligatoire au moin 1)</p></>}
                                                    </CardTitle>
                                                </CardHeader>
                                                <CardContent>

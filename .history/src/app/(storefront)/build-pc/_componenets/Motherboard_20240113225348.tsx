...
export const Motherboard = (props:{
  ...
  // Check if the current property is not 'profiles'
  filterKey !== 'profiles' && (
    <CheckboxGroup
      key={filterKey}
      label={(filter as Filter).title.toString()} // Type assertion to ensure 'filter' is of the expected type
      items={(filter as Filter).list}
  ...
  <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-4">
          {data.map((item,key) => (
           ...
           <Button className={'w-full '+`${checkcompatibility(item)? 'bg-green-400 hover:bg-green-600 ':'bg-red-400 hover:bg-red-600'}`}>Ajouter</Button>
         </div>

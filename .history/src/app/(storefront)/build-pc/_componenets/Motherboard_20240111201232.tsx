...
export const Motherboard = (props:{
  ...
  const fetchData = async () => {
    try {
      ...
      const response = await fetch(`/api/motherboard/component?units=10&page=0&filterList=${encodedFilterList}`);
  ...
  const sortOptions = [
    { name: 'Most Popular', href: '#', current: selectedSort === 'Most Popular' },
    { name: 'Best Rating', href: '#', current: selectedSort === 'Best Rating' },
    { name: 'Newest', href: '#', current: selectedSort === 'Newest' },
    { name: 'Price: Low to High', href: '#', current: selectedSort === 'Price: Low to High' },
    { name: 'Price: High to Low', href: '#', current: selectedSort === 'Price: High to Low' },
  ];
    ...
    <div>
                          ...
                          {({ active }) => (
                            ...
                            onClick={() => handleSortClick(option.name)}
                              ...
                              className={classNames(
                                option.current ? ' text-[#db8d08] font-medium  dark:text-gray-100' : 'text-gray-500',
                                active ? 'bg-gray-100  ' : '',
      ...
      <div className="w-full h-full p-8">
          ...
          <div className="mt-3 grid grid-cols-1 sm:grid-cols-2 md:grid-cols-5 gap-2">
            <Skeleton className="aspect-square h-20 rounded-xl" />
  ...
  <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-4">
          {data.map((item) => (
           <> <div className="dark:bg-[#000000e6] bg-[#ffffffe6]  group cursor-pointer rounded-xl border p-3 space-y-1">
           {/* Image & actions */}

...
export const Motherboard = (props:{
  ...
  const fetchData = async () => {
    ...
    const handleCheckboxChange = (filterKey: keyof checkItemGroups, value: string) => {
      ...
      const existingItem = updatedFilterList[filterKey].find((item) => item.searchKey === value);
  
      if (existingItem) {
        // Remove the item if it exists
        updatedFilterList[filterKey] = updatedFilterList[filterKey].filter((item) => item.searchKey !== value);
      } else {
  ...
  console.log(           props.motherboardmanufacturer.list)
  return (
    <div>
      <Dialog>
        ...
        items={props.motherboardchipset.list.map((e)=>{
          return e.name
        })}
        onChange={(value) => handleCheckboxChange('motherboardchipset', value)}
        selectedItems={filterList.motherboardchipset.map((item) => item.searchKey)}
      />
                <div>
                {ee.name} ({ee.number})
                </div>

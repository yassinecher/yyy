...
export const Ram = (props: {
    ...
    const updateElementAtIndex = (index: number, newValue: Memory | undefined) => {
        if (newValue && extractLesserNumber(newValue.name) === 2) {
          const emptySlots = props.rams.filter((e) => e === undefined);
          ...
          props.setRams((prevRamschoisi: Array<Memory | undefined>) => {
            ...
            updatedArray[emptySlots[0] ? emptySlots[0] : index + 1] = undefined;
            return updatedArray;
          });
        } else if (newValue && extractLesserNumber(newValue.name) === null) {
          props.setRams((prevRamschoisi: Array<Memory | undefined>) => {
            const updatedArray = [...prevRamschoisi];
            updatedArray[index] = newValue;
            return updatedArray;
          });
        } else if (newValue === undefined && props.rams[index] && extractLesserNumber(props.rams[index]?.name) === 2) {
          props.setRams((prevRamschoisi: Array<Memory | undefined>) => {
            const updatedArray = [...prevRamschoisi];
            updatedArray[index] = newValue;
            const emptySlots = updatedArray.filter((e) => e === undefined);
            if (emptySlots.length > 1) {
              const indexToRemove = emptySlots[0] ? updatedArray.indexOf(emptySlots[0]) : index + 1;
              updatedArray[indexToRemove] = undefined;
            }
            return updatedArray;
          });
        }

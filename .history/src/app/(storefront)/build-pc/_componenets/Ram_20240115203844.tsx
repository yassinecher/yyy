...
export const Ram = (props: {
    ...
    const addemptyCase=()=>{
            ...
            props.setRams((prevRamschoisi: Array<Memory | undefined>) => {
              ...
              updatedArray[updatedArray.length + 1] = undefined; // Assuming you want to set the next element to undefined
              return updatedArray;
            });
...
const Number = async () => {
    try {
      ...
      if (dataa.number === 2 || dataa.number === 4) {
        const filteredRams = props.rams.filter((e) => e !== undefined);
        const twoSlotProducts = filteredRams.filter((e) => e && is2caseProduct(e));
  
        if (filteredRams.length > dataa.number) {
          // Remove excess elements if the number of slots decreases
          const updatedRams = filteredRams.slice(0, dataa.number);
          props.setRams(updatedRams);
        } else if (filteredRams.length < dataa.number) {
          // Add undefined elements if the number of slots increases
          const updatedRams = Array.from({ length: dataa.number }, (_, index) => filteredRams[index] || undefined);
  
          if (dataa.number === 4) {
            if (twoSlotProducts.length === 1) {
              // If there is 1 product with 2 slots, create an array of 3
              updatedRams[2] = twoSlotProducts[0];
            } else if (twoSlotProducts.length === 2) {
              // If there are 2 products with 2 slots, create an array of 2
              updatedRams[0] = twoSlotProducts[0];
              updatedRams[1] = twoSlotProducts[1];
              updatedRams[2] = undefined; // Assuming you want the third element to be undefined
            }
          }
  ...
  const updateElementAtIndex = (index: number, newValue: Memory | undefined) => {
    if (newValue && extractLesserNumber(newValue?.name) === 2) {
      ...
      if (arr.length < 2) {
        ...
        toast.success('Ce produit a 2 barrettes RAM, veuillez libérer un autre emplacement RAM.');

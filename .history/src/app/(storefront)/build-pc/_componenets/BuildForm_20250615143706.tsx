...
export function extractNumber(text: string): number | null {
  // Regex to match one or more digits at the start of the string
  const regex = /^(\d+)/;
  const match = text.match(regex);

  if (match) {
    // Extract the numeric part (group 1) and convert to number
...
export function extractGpuSize(title: string): number | null {
  // Regex to match number followed by g, gb, or go (case-insensitive)
...
export function extractCoreCount(text: string): number | null {
  // Regex to match core count (e.g., "8 CŒURS", "14 Cores", "12 COEURS")
...
export function extractPerformanceCoreClock(text: string): number | null {
  // Normalize commas to periods for decimal numbers (e.g., "5,4" to "5.4")
  const normalizedText = text.replace(/(\d+),(\d+)/g, '$1.$2');

  // Regex to match clock speeds in GHz (e.g., "4.5 GHz / 5.4 GHz", "3.5 GHz - 5.3 GHz", "4.9 GHz")
  // Captures numbers like X.Y or X (optional decimal) followed by GHz
  const regex = /(\d+\.?\d*)\s*GHz\b/gi;
  const matches = normalizedText.match(regex);

  if (matches) {
    // Extract all numbers, convert to float, and find the maximum
    const clocks = matches
      .map(match => parseFloat(match.replace(/GHz/i, '').trim()))

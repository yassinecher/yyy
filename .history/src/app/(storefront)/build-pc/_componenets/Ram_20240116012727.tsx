...
type SetRams = (callback: (prevRams: Array<Memory | undefined>) => Array<Memory | undefined>) => void;
...
export const Ram: React.FC<YourComponentProps> = (props: {
    ...
    const [ramId,  props.setRams] = useState<Array<Memory | undefined>>(props.rams)
    const addEmptyCase = () => {
         props.setRams((prevRams: (Memory | undefined)[]) => [
          ...prevRams,
          undefined
        ]);
      };
      ...
      const removeEmptyCase = () => {
        const indexToRemove = ramId.findLastIndex((item) => item === undefined);
        if (indexToRemove !== -1) {
           props.setRams((prevRams: (Memory | undefined)[]) => {
      ...
      const updateCase = (index: number, item: Memory | undefined) => {
         props.setRams((prevRams: Array<Memory | undefined>) => {
          const updatedArray = [...prevRams];
      ...
      const updatewithremove2case=(index: number, item: Memory | undefined)=>{
         props.setRams((prevRams: Array<Memory | undefined>) => {            
            const updatedArray = [...prevRams];
            updatedArray[index] = item;
            const indexToRemove = updatedArray.findIndex((item) => item === undefined);
            if (indexToRemove !== -1) {
                updatedArray.splice(indexToRemove, 1);
            }
            return updatedArray;
      ...
      const remove2CaseProduct = (index: number) => {
        ...
        updatedArray[index + 1] = undefined;
         props.setRams((prevRams: (Memory | undefined)[]) => updatedArray);
      };
      ...
      const remove2caseProduct = (index: number) => {
        const updatedArray = [...props.rams];
        updatedArray[index] = undefined;
        updatedArray[updatedArray.length - 1] = undefined;
         props.setRams((prevRams: (Memory | undefined)[]) => updatedArray);
      };
...
const Number = async () => {
    try {
      const response =await axios.get(`/api/motherboard/RamSlots?id=${props.motherboardId?.id}`);
  ...
  const updateElementAtIndex = (ind: number, newVe: Memory | undefined) => {
    const index = ind
    ...
    if (newValue === undefined) {
      ...
      if (ramId.filter((e) => e === undefined).length >= 1) {
        // Update with the new value and remove an empty case if available
        updatewithremove2case(index, newValue);
        ...
        toast.error('Ce produit a 2 barrettes RAM, veuillez libérer un autre emplacement RAM.2');
      }
    }
...
useEffect(()=>{
    ...
    useEffect(() => {
        const filteredRams = ramId.filter((e) => e !== undefined);
        
        if (ramSlotNumber === 2) {
          // Initialize ramId with two undefined values
          
      
          let n = 0;
          let i = 0;
          let array1:( Memory|undefined)[] =[]
          ...
          for(let Xy=array1.length+1;Xy<2;Xy++){
            array1[i]=undefined
          }
           props.setRams([...array1]);
        } else if (ramSlotNumber === 4) {
          // Initialize ramId with four undefined values
           props.setRams([undefined, undefined, undefined, undefined]);
      
          // Update each slot with corresponding RAM product
          filteredRams.forEach((it, index) => {
            updateElementAtIndex(index, it);
          });
        }
      }, [ramSlotNumber, ramId, is2caseProduct, updateElementAtIndex]);

...
export const Ram = (props: {
    ...
    const addEmptyCase = () => {
        props.setRams((prevRamschoisi: Array<Memory | undefined>) => {
          return [...prevRamschoisi, undefined];
        });
      };
    const removeEmptyCase= () => {
      ...
      const add2EmptyCase = () => {
        props.setRams((prevRamschoisi: Array<Memory | undefined>) => {
          return [...prevRamschoisi, undefined, undefined];
        });
      };
      const updateCase = (index: number,item:Memory | undefined) => {
        props.setRams((prevRamschoisi: Array<Memory | undefined>) => {
            const updatedArray = [...prevRamschoisi];
      ...
      const remove2CaseProduct = (index: number) => {
        props.setRams((prevRamschoisi: Array<Memory | undefined>) => {
          ...
          updatedArray[index + 1] = undefined;
          return updatedArray;
        });
      };

const is2caseProduct=(item:Memory)=>{
   return extractLesserNumber(item?.name)==2
}

const remove2caseproduc=(index:number)=>{
    props.setRams((prevRamschoisi: Array<Memory | undefined>) => {
        const updatedArray = [...prevRamschoisi];
        updatedArray[index] = undefined;
        updatedArray[updatedArray.length ] = undefined; // Assuming you want to set the next element to undefined
        return updatedArray;
      });
}
...
const Number = async () => {
    try {
      ...
      const filteredRams = props.rams.filter((e) => e !== undefined);
  
      if (dataa.number === 2) {
        if (filteredRams.length <=2) {
            let n = 0;
            filteredRams.forEach((it, kk) => {
                if (it && is2caseProduct(it)) {
                  n = n + 2;
                } else {
                  n = n + 1;
                }
              });
              const updatedRams = Array.from({ length: 2 }, (_, index) => filteredRams[index] || undefined);
              props.setRams([undefined, undefined]);
           
              if (n <= 2) {
                updatedRams.forEach((item, index) => {
                    if(item!=undefined){
                        updateElementAtIndex(index, item);
                        console.log( index)
                    }
      ...
      } else if (dataa.number === 4) {
       
          let n = 4;
          props.setRams([undefined, undefined,undefined, undefined]);


          filteredRams.forEach((item, index) => {
            updateElementAtIndex(index, item);
          });
          const updatedRams = Array.from({ length: n }, (_, index) => filteredRams[index] || undefined);
      }
    } catch (error) {
      setLoading(false);
      console.error('Error fetching data:', error);
...
const updateElementAtIndex=(index: number, newValue: Memory | undefined)=>{
    ...
    if(newValue===undefined){
        if(prvprod===undefined){
           //leave empty

        }else if(prvprod&&is2caseProduct(prvprod)){
          //ajout
          updateCase(index,undefined)
          addEmptyCase()

        }else{
            updateCase(index,undefined)  
        }
    }else if (prvprod==undefined){
        if(is2caseProduct(newValue)){
            if(props.rams.filter((e)=>e===undefined).length>=2){
        ...
        if(props.rams.filter((e)=>e===undefined).length>=1){
            updateCase(index,newValue)
            removeEmptyCase()
        }
    }
  ...
  const updateElementAtIndexx = (index: number, newValue: Memory | undefined) => {
    if (newValue && extractLesserNumber(newValue?.name) === 2) {
      const arr = props.rams.filter((e) => e == undefined);
      if (arr.length < 2) {
        const name = props.rams[index];
            if(name&&is2caseProduct(name)){
                props.setRams((prevRamschoisi: Array<Memory | undefined>) => {
                    const updatedArray = [...prevRamschoisi];
                ...
                if(name&&arr.length==1){
                    props.setRams((prevRamschoisi: Array<Memory | undefined>) => {
                     ...
                     toast.error('Ce produit a 2 barrettes RAM, veuillez libérer un autre emplacement RAM.');
                }
               
            }
        
      } else {
        toast.success('Ce produit a un autre emplacement RAM.');
  
        const filteredArray = arr.filter(Boolean);

        // Set the new value at the specified index
        filteredArray[index] = newValue;
  
        props.setRams(filteredArray);
      }
    } else {
      ...
      if(newValue === undefined&& props.rams[index]===undefined){

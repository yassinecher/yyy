                      ...
                      {data.map((item, key) => (
                        ...
                        <div key={key} className={`${checkcompatibility(item) ? 'border-green-600 ' : 'border-red-400'}` + " dark:bg-[#321642d0]   bg-[#ffffffe6] flex flex-col justify-between group cursor-pointer rounded-xl border p-3 space-y-1"}>
                          ...
                          <div className='w-full'>
                                                    ...
                                                    <Button type='submit' onClick={() => { props.setProcessorId(item);props.addFeature("Power Supply_Wattage",item.
powersupplies[0].Power ) ;steOpenDialog(false) }} className={'w-full ' + `${checkcompatibility(item) ? 'bg-green-400 hover:bg-green-600 ' : 'bg-red-400 hover:bg-red-600'}`}>Ajouter</Button>

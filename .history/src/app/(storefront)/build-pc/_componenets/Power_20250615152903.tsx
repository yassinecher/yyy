...
export const Power = (props: {
  ...
  const [filterList, setFilterList] = useState<checkItemGroupsPower>({
    ...
    const [recommendedps,setRecommendedps]= useState();
    ...
    try {
      ...
      setLoading(false);
          const res= await getRecommendations(props.selectedFeatures);
                 setRecommendedps(res.recommendations[0].powerSupplyWattage) 
      console.log(dataa);
    } catch (error) {
      setLoading(false);
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-4">
                      {data.map((item, key) => (
                        ...
                        <div key={key} className={`${checkcompatibility(item) ? 'border-green-600 ' : 'border-red-400'}` + " dark:bg-[#321642d0]   bg-[#ffffffe6] flex flex-col justify-between group cursor-pointer rounded-xl border p-3 space-y-1"}>
                          {/* Image & actions */}
                          <div>
                             {item.powersupplies[0].Power === recommendedps && (
                <span className="text-blue-500 font-semibold">Recommandé par AI</span>
              )}
                          ...
                          <div className='w-full'>
                                                    ...
                                                    <Button type='submit' onClick={() => { props.setProcessorId(item);props.addFeature("Power Supply_Wattage",item.powersupplies[0].Power );console.log(props.selectedFeatures) ;steOpenDialog(false) }} className={'w-full ' + `${checkcompatibility(item) ? 'bg-green-400 hover:bg-green-600 ' : 'bg-red-400 hover:bg-red-600'}`}>Ajouter</Button>

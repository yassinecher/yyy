...
export const Motherboard = (props: {
  ...
  const fetchData = async () => {
    try {
      ...
      const response = await fetch(`/api/motherboard/component?minDt=${priceFilter[0]}&maxDt=${priceFilter[1]}&q=${searchTerm}&sort=${selectedSort}&units=10&page=${currentPage}&filterList=${encodedFilterList}`);
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-4">
                      {data.map((item, key) => (
                        ...
                        <div key={key} className="dark:bg-[#000000e6] bg-[#ffffffe6] flex flex-col justify-between group cursor-pointer rounded-xl border p-3 space-y-1">
                          ...
                          <Popover>
  <PopoverTrigger><Button variant={'ghost'} className='my-2 w-full'>Details</Button></PopoverTrigger>
  <PopoverContent>

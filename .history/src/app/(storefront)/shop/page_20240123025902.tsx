...
import { addcpuFitlters, addmotherboardFitlters } from './_componenets/FilterFunctions'
...
const Home = async ({
  ...
  if (filterListParam.length > 0) {
    ...
    const type = decodedFilterList.type
    if (decodedFilterList.data) {
      fList = decodedFilterList.data
      if (type == FilterType.motherboard) {
        whereClause.motherboard = addmotherboardFitlters(fList).data
        console.log(type)
      }
      if (type == FilterType.cpu) {
        whereClause.cpu = addcpuFitlters(fList).data
      }
      if (type == FilterType.gpu) {
        whereClause.gpu = addgpuitlters(fList).data
     
      }


      if (type == FilterType.ram) {

      }



      if (type == FilterType.hardDisk) {

      }



      if (type == FilterType.mic) {

      }



      if (type == FilterType.casque) {

      }



      if (type == FilterType.mouse) {

      }



      if (type == FilterType.mousePad) {

      }



      if (type == FilterType.cooling) {
      ...
      if (type == FilterType.power) {

      }



      if (type == FilterType.screen) {

      }



      if (type == FilterType.keyboard) {

      }



      if (type == FilterType.laptop) {

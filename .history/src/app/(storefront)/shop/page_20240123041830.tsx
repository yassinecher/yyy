...
const Home = async ({
  ...
  if (filterListParam.length > 0) {
    ...
    const typee = decodedFilterList.type as unknown as FilterType
    if (decodedFilterList.data) {
      fList = decodedFilterList.data 
      if (typee === FilterType.motherboard) {
        whereClause.motherboard = addmotherboardFitlters(fList).data
      
      }
      if (typee === FilterType.cpu) {
        whereClause.cpus = addcpuFitlters(fList).data
      }
      if (typee === FilterType.gpu) {
        whereClause.gpus = addgpuitlters(fList).data
        console.log(whereClause)
      }


      if (typee === FilterType.ram) {

      }



      if (typee == FilterType.hardDisk) {

      }



      if (typee == FilterType.mic) {

      }



      if (typee == FilterType.casque) {

      }



      if (typee == FilterType.mouse) {

      }



      if (typee == FilterType.mousePad) {

      }



      if (typee == FilterType.cooling) {

      }



      if (typee == FilterType.case) {

      }



      if (typee == FilterType.power) {

      }



      if (typee == FilterType.screen) {

      }



      if (typee == FilterType.keyboard) {

      }



      if (typee == FilterType.laptop) {

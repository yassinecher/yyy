...
const [filterList, setFilterList] = useState<FilterList>(selectfilterList||{})
  ...
  const fetchData=(cate?:string,sort?:string)=>{
    ...
    const encodedFilterList = encodeURIComponent(JSON.stringify(fil));
          
    router.push(`/shop?minDt=${priceFilter[0]}&maxDt=${priceFilter[1]}&search=${searchTerm}&categorie=${cate}&sort=${sort}&filterList=${encodedFilterList}`)
    router.refresh()
  }
  return (
        ...
        <div key={item.name}>
          <div className='flex items-center appearance-none'>
            <Input type='checkbox' className='appearance-none forced-colors  focus:outline-none focus-visible:outline-none w-3 h-3 m-2 outline-none'
              value={item.name}
              checked={props.selectedItems[props.label]&&props.selectedItems[props.label].findIndex((e)=>e.searchKey===item.name)!=-1}
              onChange={() => props.onChange(item.name)}
            />
            <label className='text-sm'>{item.name} ({item.number})
            {props.selectedItems[props.label]&&props.selectedItems[props.label].findIndex((e)=>e.searchKey===item.name)!=-1?'true':"false"}
            </label>
            <div>

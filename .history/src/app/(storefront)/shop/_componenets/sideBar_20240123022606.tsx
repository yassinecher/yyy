...
const [filterList, setFilterList] = useState<FilterList>(selectfilterList||{})
  ...
  const fetchData=(cate?:string,sort?:string)=>{
    ...
    router.push(`/shop?minDt=${priceFilter[0]}&maxDt=${priceFilter[1]}&search=${searchTerm}${cate?"&categorie="+cate:""}&sort=${sort}${encodedFilterList.length>0?"&filterList="+encodedFilterList:""}`)
    router.refresh()
  }
  return (

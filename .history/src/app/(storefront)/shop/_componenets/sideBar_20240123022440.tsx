...
const [filterList, setFilterList] = useState<FilterList>(selectfilterList||{})
  ...
  const fetchData=(cate?:string,sort?:string)=>{
    ...
    if(!cate&&!cat){
      ...
      if(cat){
        const fil:compFilter={
          data:filterList,
          type:FilterType.Motherboard
        }
       encodedFilterList = encodeURIComponent(JSON.stringify(fil));
       cate=cat
      }
    ...
    router.push(`/shop?minDt=${priceFilter[0]}&maxDt=${priceFilter[1]}&search=${searchTerm}${cate.length>0?"&categorie="+cate:""}&sort=${sort}${encodedFilterList.length>0?"&filterList="+encodedFilterList:""}`)
    router.refresh()
  }
  return (
        ...
        <main className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
          <div className="flex items-baseline justify-between border-b border-gray-200 dark:border-gray-800 pb-6 pt-24">
            ...
            {header!='Store'?(
              ...
              <><span className='font-light text-sm underline cursor-pointer' onClick={()=>{router.push("/shop");router.refresh()}}> Tous Les produits {"> "} </span><span className='font-light text-sm '>{header}</span>

...
const Sidebar: React.FC<ProductListProps> = ({
  ...
  const fetchData=()=>{
    router.push(`/shop?search=${searchTerm}&categorie=${category}`)
    router.refresh()
  }
  return (
...
<div>
      <input
       className='w-full border rounded-sm !border-amber-500 focus:!border-amber-900 '
        type="text"
        placeholder="Search..."
        value={searchTerm}
      ...
      <div className='w-full'>
        ...
        <PriceFilter setLoading={setIsloading} value={priceFilter} handlePriceFilter={setPriceFilter} />
              ...
              <div className="lg:col-span-3"> <div className="space-y-4">
                <h3 className="font-bold text-3xl">{title.length>0?<>Search :{title} </>:<></>}<span className='text-sm font-normal'>{
            timeTaken>0?<>{totalprod}  Résultats en {timeTaken.toFixed(2)} seconds</>:<>

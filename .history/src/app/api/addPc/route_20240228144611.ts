      ...
      data: 
      {
        ...
        pcTemplate:{
            create:{
              ...
              motherBoardId: {
                connectOrCreate: motherBoardId.map((motherboard: string) => ({
                  productId:motherboard
                })),
              },
              processorId: {
                connectOrCreate: processorId.map((cpu: string) => ({
                  productId:cpu
                })),
              },
              graphicCardId: {
                connectOrCreate: graphicCardId.map((gpu: string) => ({
                  productId:gpu
                })),
              },
              powerSupplyId: {
                connectOrCreate: powerSupplyId.map((powerSupply: string) => ({
                  productId:powerSupply
                })),
              },
              caseId: {
                connectOrCreate: caseId.map((pcCase: string) => ({
                  productId:pcCase
                })),
              },
              cooling: {
                connectOrCreate: coolings.map((pcCase: string) => ({
                  productId:pcCase
                })), 
              },
              ramIdArray: {

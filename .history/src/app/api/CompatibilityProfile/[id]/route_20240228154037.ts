      ...
      await prisma.componentOnPc.deleteMany({
        where: {
          OR: [
            ...
            { id: { in: compatibilityProfile.coolings.map((psu) => psu.id) } },
...
export async function PATCH(
    ...
    try {
        ...
        const { name,coolingList, motherboardList, processorList, graphicCardList, powersupplyList, ramSlotList, storageList, caseList } = body;
      ...
      await prisma.componentOnPc.deleteMany({
        where: {
          OR: [
            ...
            { id: { in: compatibilityProfile.CPUs.map((cpu) => cpu.id) } },
            { id: { in: compatibilityProfile.GPUs.map((gpu) => gpu.id) } },
            { id: { in: compatibilityProfile.coolings.map((gpu) => gpu.id) } },
            { id: { in: compatibilityProfile.powersupplys.map((psu) => psu.id) } },
            { id: { in: compatibilityProfile.cases.map((pcCase) => pcCase.id) } },
            { id: { in: compatibilityProfile.RAMs.map((ramSlot) => ramSlot.Components.map((ram) => ram.id)).flat() } },
            { id: { in: compatibilityProfile.disks.map((diskSlot) => diskSlot.Components.map((disk) => disk.id)).flat() } },
      ...
      await prisma.componentOnPcGroupe.deleteMany({
        where: {
          OR: [
            { id: { in: compatibilityProfile.RAMs.map((mb) => mb.id) }  },
            { id: { in: compatibilityProfile.disks.map((mb) => mb.id) }  },
          ],
        },
      });
      ...
      const newCompatibilityProfile = await prisma.compatibiltyProfile.update({
        ...
        data: {
          ...
          coolings: {
            create: coolingList.map((cpu: any) => ({
              productId:cpu
            })),
          },
          GPUs: {
          ...
          RAMs: {
            create: ramSlotList.map((ramSlot: { rams: any[] ,defaultId:string}) => ({
              Components: {
          ...
          disks: {
            create: storageList.map((storage: { disks: any[],defaultId:string }) => ({
              Components: {

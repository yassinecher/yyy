...
export async function POST(
  ...
  try {
    ...
    let htmlContent = fs.readFileSync('mail/mail-model.html', 'utf-8');
    const prodshtml=genratehtml(productIdss)
    htmlContent.replace("${prodssss}",prodshtml)    
    htmlContent.replace("${itemNumber}",productIdss.length)
    htmlContent.replace("${TotaLPrice}",totalPrice)
    htmlContent.replace("${ville}",ville)
    htmlContent.replace("${rue}",rue)
    htmlContent.replace("${codePostal}",codePostal)

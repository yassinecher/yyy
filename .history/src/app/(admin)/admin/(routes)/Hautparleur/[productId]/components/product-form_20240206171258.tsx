...
import { Category, Image, Product, Manufacturer, RamSlots, MotherboardChipset, CPUSupport, Guarantee, MotherboardFormat, Motherboard, Field, Laptop, LapRefreshRate, LapCamera, LapSound, Lapnetwork, Lapmemory, LapHardisk, LapScreenType, LapScreenSize, LapGraphiccardRef, LapGraphiccard, LapProcesseurRe, LapProcesseur, LapSystem, keyboard, keyboarButtonsNumber, keyboarFormat, keyboarTouchType, keyboarbrand, Headset, HeadsetModel, HeadsetSonSurround, HeadsetInterfaceAvecOrdinateur, RgbType, Sonsurround, Hautparleur } from "@prisma/client"
...
export const ProductForm: React.FC<ProductFormProps> = ({
  initialData,
  categories, RgbType, Sonsurround,manufacturers
}) => {
  ...
  const defaultValues = initialData  && initialData.Hautparleur ? {
    ...initialData,
    price: parseFloat(String(initialData?.price)),
    dicountPrice: parseFloat(String(initialData?.dicountPrice)),
    stock: parseFloat(String(initialData?.stock)),
    SonsurroundId:initialData.Hautparleur[0].SonsurroundId??'',
    RgbTypeId:initialData.Hautparleur[0].RgbTypeId??'',
    manufacturerId:initialData.Hautparleur[0].manufacturerId??'',
...
console.log(initialData)
  ...
  const onSubmit = async (data: ProductFormValues) => {
    ...
    try {
      ...
      if (initialData) {
        await axios.patch(`/api/peripherique/Hautparleur/component/${params.productId}`, data);
      } else {
        await axios.post(`/api/peripherique/Hautparleur/component`, data);
      }
      router.refresh();
      router.push(`/admin/Hautparleur`);
  ...
  return (
    <>
      ...
      <Separator />
      <Form {...form}>
        <form onSubmit={(e)=>{e.preventDefault();form.handleSubmit(onSubmit);console.log(form.getFieldState("RgbTypeId")) }} className="space-y-8 w-full">
        <FormField
...
<PopFormModal label={"Son Surround"} 
              ...
              data={Sonsurround}
              fieldaAfficher="name"
              url="/api/peripherique/Hautparleur/Sonsurround"
              formLab="SonsurroundId"
              formCControlName="headsetSonSurroundId"
              IsNumber={false}
              />
              
          <PopFormModal label={"Rgb Type"} 
              form1={form} 
              loading={loading} 
              setLoading={setLoading} 
              data={RgbType}
              fieldaAfficher="name"
              url="/api/peripherique/RgbType"
              formLab="RgbTypeId"
              formCControlName="RgbType"
              IsNumber={false}
              />

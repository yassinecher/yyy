...
type motherboardpar = {
  ...
  setMotherBoardId: (value: String) => void;
}

const ProductCOl: React.FC<motherboardpar> = ({ init, colname, motherboards, selectionStaue, setMotherBoardId, onSlectionChange, updateList, MotherBoardId, lastMotherBoardId, setLastMotherBoardId }) => {
  ...
  useEffect(() => {
    if ( motherboardList.length > 0) {
      if(MotherBoardId.length>0){
         if(motherboardList.find((e)=>e.id==MotherBoardId)){
          setMotherBoardId(MotherBoardId )
         }
      }else{
         setMotherBoardId(motherboardList[0].id )
      }
              ...
              <div className='w-full pb-3 group '>
                {
                  item.images.length > 0 ? <>
                   ...
                   {item.name} <br />
                   {item.price} TND <br />
                 {MotherBoardId!= item.id?<>
                  <button type='button' className='w-auto bg-green-500 m-2  text-white p-2 rounded-sm' onClick={()=>setMotherBoardId(item.id)}> Set Default</button>
                 </>:<>

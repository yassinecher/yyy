    ...
    const closeDialogRam = async () => {
      if (form.getValues().ramslots) {
        try {
          ...
          setLoading(true); // Define 'setLoading' somewhere
  
          let resp = await axios.post(`/api/motherboard/RamSlots`, { number });
          // Assuming 'ramslots' and 'toastMessage' are defined
          ramslots.push(resp);
          toast.success(toastMessage);
          router.refresh(); // Define 'router' using useRouter from your routing library
          setIsDialogOpenRam(false);
        } catch (error: any) {
          toast.error('Something went wrong: ' + error);
          setIsDialogOpenRam(false);
        } finally {
          setLoading(false); // You should reset 'setLoading' here
          setIsDialogOpenRam(false);
        }
      } else {

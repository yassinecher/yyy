...
export const ProductForm: React.FC<ProductFormProps> = ({
  ...
  const closeDialogRam = async () => {
    ...
    if (form.getValues().ramslots) {
      try {
      
          let number =parseInt(form.getValues().ramslots!.toString(),10)
        setLoading(true);

        let resp= await axios.post(`/api/motherboard/RamSlots`, {  number});
        ramslots.push(resp)
        toast.success(toastMessage);
        router.refresh();
        setIsDialogOpenRam(false);
      } catch (error: any) {
        toast.error('Something went wrong.');
        setIsDialogOpenRam(false);
      } finally {
      ...
      form.setError('ramslots', { message: 'Insert the name' });
      setIsDialogOpenRam(false);
    }
    ...
    <>
      ...
      <Form {...form}>
        <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-8 w-full">
          ...
          <div className="md:grid md:grid-cols-3 align-middle items-center gap-8">
...
<PopFormModal label={"Motherboard Chipset"} form1={form} loading={loading} setLoading={setLoading} data={ramslots}  />
          ...
          <FormField
            ...
            render={({ field }) => (
              <FormItem  >
                <FormLabel>CPUSupport</FormLabel>
                <div className="md:grid md:grid-cols-2 align-middle items-center gap-8">
                  <div>
                    <Select disabled={loading} onValueChange={ field.onChange } value={field.value} defaultValue={field.value}>
                      <FormControl>
          ...
          <FormField
            ...
            render={({ field }) => (
              <FormItem  >
                <FormLabel>Motherboard Format</FormLabel>
                <div className="md:grid md:grid-cols-2 align-middle items-center gap-8">
                  <div>
                    <Select disabled={loading} onValueChange={ field.onChange } value={field.value} defaultValue={field.value}>
                      <FormControl>

...
export const NavbarList: React.FC<NavbarCollection> = ({
    ...
    if (navList.length < 6) {
        try {
            ...
            const nav = {
              ...
              await axios.post(`/api/mobile/navitem`, { nav });
  ...
  const onSubmit = async () => {
    try {
      ...
      await axios.post(`/api/mobile/navbar`, { NavbarCollection: data }).finally(()=>{
   router.refresh();
      });
    ...
    <>
      <div>
        <div className="flex items-center justify-between">
          <Heading title="Mobile Navbar" description="Manage Navbar for your store" />

...
console.log(data)
  ...
  {NavCollectons.map((collection, index) => (
    <TabsTrigger value={index.toString()} className="flex justify-between align-middle  h-7">
     <Button variant={"ghost"}  className="hover:bg-slate-400 w-3 h-5">{"<"}</Button> {collection.Label.length === 0 ? `Label ${index + 1}` : collection.Label}

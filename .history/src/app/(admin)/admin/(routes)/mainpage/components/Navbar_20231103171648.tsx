...
console.log(data)
  ...
  {NavCollectons.map((collection, index) => (
    <TabsTrigger value={index.toString()} className="flex justify-between">
     <Button variant={"ghost"}  className="hover:bg-slate-100 w-3 h-3">{"<"}</Button> {collection.Label.length === 0 ? `Label ${index + 1}` : collection.Label}
     <Button variant={"ghost"} className="w-3 h-3">{">"}</Button></TabsTrigger>
  ))}
</TabsList>
   {
            NavCollectons.map(

...
const onSubmit= async()=>{
  ...
  try {
    let data=NavCollectons.map((val,k)=>val.index=k)
    await axios.post(`/api/navbar`, { NavbarCollection: NavCollectons });
    router.refresh()
  ...
  {NavCollectons.map((collection, index) => (
    <TabsTrigger key={index} onClickCapture={()=>setNavigatorValue(index.toString())} value={index.toString()} className="flex justify-between align-middle p-0 h-8">
...
</TabsList>
   {
        ...
        <Card>
          <CardHeader>
            <CardTitle className="flex align-middle" ><div className="flex align-middle"> {
                i.Label.length==0?<>  <div> Label {(k+1).toString()}</div> </>:<><div className="my-auto">{ i.Label}</div></>
            }<IconButton onClick={()=>removelabel(k)} className="bg-red-500 rounded-md ml-3 "  icon={<Trash size={20} className="text-white" />}
            ></IconButton></div></CardTitle>
            <CardDescription>
    ...
    <Dialog data={categories} id={currentDialgId} handlechange={(i,g)=>{handleupdate(i,g)}} onClose={()=>{setDialogOpen(false)}} openDialg={dialogOpen} prodList={[]}/>
         ...
         </Tabs>    <Button onClick={()=>{onSubmit()}}>Save changes</Button>
    </>
  );
};

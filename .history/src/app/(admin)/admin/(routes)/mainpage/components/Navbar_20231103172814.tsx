...
console.log(data)
  ...
  {NavCollectons.map((collection, index) => (
    <TabsTrigger value={index.toString()} className="flex justify-between align-middle p-0 h-8">
     ...
     <Button variant={"ghost"} disabled={NavCollectons.length === 1} className="flex justify-center align-middle hover:bg-slate-400 w-3 h-5 py-2 pb-2 mx-3">{">"}</Button></TabsTrigger>
  ))}
</TabsList>
   {
            NavCollectons.map(

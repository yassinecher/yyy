...
type InputArrayProps = {
  ...
  inputArrayp:  Array<{ name: string; value: string }>;
};

function InputArray({ onInputArrayChange, inputArrayp }: InputArrayProps) {
    ...
    <div className="space-y-4">
      {inputArray.map((input, index) => (
        <div key={index} className="flex space-x-4 items-center">
          ...
          >
            <Trash  onClick={() =>{ removeInput(index);exportInputArray}}/>
          </Button>
        </div>
      ))}
      <Button

...
export const ProductForm: React.FC<ProductFormProps> = ({
  ...
  const [cpu,setcpu]=useState<ProdCol[]>(motherboards.filter((e)=>initialData?.CPUs.find((ee)=>e.id==ee.productId)))
  const [gpu,setgpu]=useState<ProdCol[]>(motherboards.filter((e)=>initialData?.GPUs.find((ee)=>e.id==ee.productId)))
  const [power,setPower]=useState<ProdCol[]>(motherboards.filter((e)=>initialData?.powersupplys.find((ee)=>e.id==ee.productId)))
  const [ramSlot,setramslot]=useState<ramSlot[]>([{rams:[]}])
  const [casesl,setcase]=useState<ProdCol[]>(motherboards.filter((e)=>initialData?.cases.find((ee)=>e.id==ee.productId)))

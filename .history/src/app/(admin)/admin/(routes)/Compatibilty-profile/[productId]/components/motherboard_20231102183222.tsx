  ...
  return (
    <div>
        ...
        <div className='sm:w-full md:w-96 w-96'>
            ...
            {filteredMotherboards.length>0?         (<>        <div className="rounded-md p-2 w-min flex border-slate-500  border-2 col-span-4 col-">{filteredMotherboards.map((prod: { id: React.Key | null | undefined; images: string | any[]; name: string | number | boolean | React.ReactElement<any, string | React.JSXElementConstructor<any>> | Iterable<React.ReactNode> | React.ReactPortal | React.PromiseLikeOfReactNode | null | undefined }) => (
          <div key={prod.id}>
            {prod.images.length > 0 ? (
              <Image src={prod.images[0].url} className='rounded-none' width={40} height={40} />
            ) : (
              <></>
            )}
            <span>{prod.name}</span>

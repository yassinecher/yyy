    ...
    <>
      ...
      <Form {...form}>
        ...
        setramslot(updatedRamSlot);
            ...
            <MotherboardCOl  colname="cpu" motherboards={rams}  updateList={(e)=>{
              
              const newram=ramSlot[key].rams=e
              setcase(newram)
              console.log(newram)
          ...
          <div>
          <div className='font-semibold my-3 flex items-center'>rams <Button onClick={()=>setDiskSlot([...DiskSlot,{disk:[]}])} variant={'outline'} className="mx-3">Add new Ram Slot</Button></div>
          {
            DiskSlot.map((item,key)=>{
              return (<>
               <div>
            <div className='font-semibold flex items-center'>Disk slot {key+1}{ramSlot.length>1?<Button onClick={() => {
        const updatedDiskSlot = DiskSlot.filter((_, index) => index !== key);
        setDiskSlot(updatedDiskSlot);
      }} variant={'destructive'} className="mx-3"><Trash className="w-5" /></Button>:<></>} </div>
            <MotherboardCOl  colname="cpu" motherboards={disk}  updateList={(e)=>{
              
              const dis=DiskSlot[key].disk=e
              setDiskSlot(dis)
              console.log(dis)
              }} selectionStaue={selectionStatu} onSlectionChange={(e)=>setSelectionStatu(e)} />

...
const MotherboardCOl: React.FC<motherboardpar> = ({motherboards}) => {
  ...
  return (
    <div>
        ...
        <div className='m-5 mx-0 p-3 border-1 border-slate-200  dark:border-slate-800 rounded-lg grid grid-cols-6 gap-3 col-span-6'>
            {motherboardList.map((item)=>{
                return (
                    <><div className='flex flex-col justify-end  bg-slate-100 rounded-lg  text-slate-800  dark:bg-slate-800 dark:text-slate-100'>
                    ...
                    <div className='w-full pb-3 '>
                       {
                        item.images.length>0? <>
                         <Image src={item.images[0].url} width={'100%'} height={50} className='w-full' />
...
<g>
	...
	<circle cx="139" cy="133" r="40.5"/>
            ...
            <div onClick={()=>setOpenDialg(true)}  className='dark:text-slate-100 cursor-pointer h-52 w-36 bg-slate-100 rounded-lg  text-slate-800  dark:bg-slate-800 justify-center flex align-middle items-center '>

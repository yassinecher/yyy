...
const cpuidList=cpuData.map((val)=>{
  if(val.cpusupport.id==mb?.cpusupport.id){
    return val.products[0].id
  }
})
const lab=cpuidList.map((val)=>{
  if(cpus.find((val1)=>val1.id==val)){
    return  cpus.find((val1)=>val1.id==val)
  }
return 

        ...
        <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-8 w-full">
          ...
          <div className='font-semibold my-3 flex items-center'>rams <Button onClick={()=>setramslot([...ramSlot,{rams:[]}])} variant={'outline'} className="mx-3">Add new Ram Slot</Button></div>
          {
            ramSlot.map((item,key)=>{
              return (<>
               <div>
            <div className='font-semibold flex items-center'>Ram slot {key+1}{ramSlot.length>1?<Button onClick={()=>setramslot([...ramSlot.slice(key,key+1)])} variant={'destructive'} className="mx-3"><Trash className="w-5" /></Button>:<></>} </div>

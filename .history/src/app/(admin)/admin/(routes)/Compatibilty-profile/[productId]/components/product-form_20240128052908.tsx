...
const formSchema = z.object({
  ...
  selectionStatu:z.object({ 
    ...
    coolingIsSelected:      z.boolean().default(false).optional(), })
});
...
export const ProductForm: React.FC<ProductFormProps> = ({
  ...
  const defaultValues = initialData ? {
    ...
    selectionStatu: {
      ...
      coolingIsSelected:initialData.motherboards.length>0,
    }
  } : {
    selectionStatu: {
  ...
  const [coolings,setCooling]=useState<ProdCol[]>(cooling.filter((e)=>initialData?.coolings.find((ee)=>e.id==ee.productId)))
...
for (const key in ramSlot) {
  ...
  try {
    ...
    const data={
      ...
      coolingList:[...coolings.map((val)=>val.id)],
    ...
    if (initialData) {
      await axios.patch(`/api/compatibilityProfile/${params.productId}`, data);
    } else {
...
const [motherboardId,setMotherBoardId]=useState("")
 ...
 <div>
          ...
          <div>
            ...
            <MotherboardCOl init={coolings} MotherBoardId={motherboardId} lastMotherBoardId={LastmotherboardId} colname="cooling" motherboards={cooling}  updateList={(e)=>setCooling(e)} selectionStaue={selectionStatu} onSlectionChange={(e)=>setSelectionStatu(e)} />

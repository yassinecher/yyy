...
export const ProductForm: React.FC<ProductFormProps> = ({
  ...
  const [mbs,setMbss]=useState<ProdCol[]>([])
  ...
  const [cpuinList,setcpuinList]=useState<ProdCol[]>([])
  const setMbs=(par:ProdCol[])=>{
    setMbss(par)
    if(par.length>0){
 const id= par[0].id
 const mb = motherboardData.find((val) => val.products[0].id === id);
 const cpuidList = cpuData
   .filter((val) => val.cpusupport.id === mb?.cpusupport.id)
   .map((val) => val.products[0].id);
 
 const lab = cpuidList
   .filter((val) => cpus.find((val1) => val1.id === val))
   .map((val) => cpus.find((val1) => val1.id === val))
   .filter((val) => val !== undefined) as ProdCol[];
      ...
      <Form {...form}>
        <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-8 w-full">
          ...
          <div>
            ...
            <MotherboardCOl  colname="cpu" motherboards={cpuinList}  updateList={(e)=>setcpu(e)} selectionStaue={selectionStatu} onSlectionChange={(e)=>setSelectionStatu(e)} />

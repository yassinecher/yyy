...
const onSubmit = async () => {
  ...
  for (const key in ramSlot) {
    ...
    if (ra.length === 0) { // Assuming you want to check the "rams" property of the first item
      toast.error('Add rams in slot'+(key+1));
      return; // This will exit the loop and stop further iterations
    }
  }
    ...
    <>
      ...
      <Form {...form}>
        ...
        setramslot(updatedRamSlot);
          ...
          <div>
          <div className='font-semibold my-3 flex items-center'>Disks <Button onClick={()=>setDiskSlot([...DiskSlot,{disk:[]}])} variant={'outline'} className="mx-3">Add new Ram Slot</Button></div>
          {
        ...
        setDiskSlot(updatedDiskSlot);
            ...
            <MotherboardCOl  colname="cpu" motherboards={diks}  updateList={(e)=>{

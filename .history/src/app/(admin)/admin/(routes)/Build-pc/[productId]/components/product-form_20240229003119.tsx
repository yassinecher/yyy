...
export const ProductForm: React.FC<ProductFormProps> = ({
  ...
  const defaultValues = initialData ? {
    ...
    defaultcooling:initialData.PreBuiltPcmodel?.pcTemplate.defaultcooling,
  } : {
    name: '',
    images: [],
    price: 0,
  ...
  const [defaultcooling,setDefaultcooling]= useState<String>(initialData?.PreBuiltPcmodel?.pcTemplate.defaultcooling??"")

  const [defaultCASE,setDefaultCASE]= useState<String>(initialData?.PreBuiltPcmodel?.pcTemplate.defaultcaseId??"")
  const [defaultDISK,setDefaultDISKK]= useState<String[]>(initialData?.PreBuiltPcmodel?.pcTemplate.hardDiskArray.map((e)=>e.defaultId)??[])
  const [defaultRAM,setDefaultRAMM]= useState<String[]>(initialData?.PreBuiltPcmodel?.pcTemplate.ramIdArray.map((e)=>e.defaultId)??[])
  
  useEffect(()=>{
if(initialData){
  
  initialData?.PreBuiltPcmodel?.pcTemplate.ramIdArray.forEach((e,k)=>{
    setDefaultRAM(e.defaultId,k)
  })
    initialData?.PreBuiltPcmodel?.pcTemplate.hardDiskArray.forEach((e,k)=>{
      setDefaultDISK(e.defaultId,k)
     })
}
  },[])
...
const CalculeTotal=()=>{
  ...
  if(defaultcooling.length>0){
    const prod=coolings.find((e)=>e.id==defaultcooling)
    if(prod)
    pr+=prod?.price
  }

...
interface ProductFormProps {
  initialData: Product & {
    ...
    caseId: ComponentOnPc[],
    processorId:ComponentOnPc[],
    cooling:ComponentOnPc[],
    graphicCardId:ComponentOnPc[],
    motherBoardId:ComponentOnPc[],
    powerSupplyId:ComponentOnPc[],
    ramIdArray: { Components: ComponentOnPc[] }[];
    hardDiskArray:{ Components: ComponentOnPc[] }[],
  } | null;
  categories: Category[];
...
export const ProductForm: React.FC<ProductFormProps> = ({
  ...
  const [mbs,setMbs]=useState<ProdCol[]>(motherboards.filter((e)=>initialData?.motherBoardId.find((ee)=>e.id==ee.productId)))
  const [cpu,setcpu]=useState<ProdCol[]>(cpus.filter((e)=>initialData?.processorId.find((ee)=>e.id==ee.productId)))
  const [gpu,setgpu]=useState<ProdCol[]>(gpus.filter((e)=>initialData?.graphicCardId.find((ee)=>e.id==ee.productId)))
  const [coolings,setCooling]=useState<ProdCol[]>(cooling.filter((e)=>initialData?.cooling.find((ee)=>e.id==ee.productId)))
  
  const [power,setPower]=useState<ProdCol[]>(powersupplies.filter((e)=>initialData?.powerSupplyId.find((ee)=>e.id==ee.productId)))
  const [ramSlot,setramslot]=useState<ramSlot[]>(initialData?.ramIdArray.map((ee)=>({
    rams:rams.filter((e)=>ee.Components.find((x)=>x.productId==e.id))
  }))??[{rams:[]}])
  const [casesl,setcase]=useState<ProdCol[]>(cases.filter((e)=>initialData?.caseId.find((ee)=>e.id==ee.productId)))
  const [DiskSlot,setDiskSlot]=useState<DiskSlot[]>(initialData?.hardDiskArray.map((ee)=>({

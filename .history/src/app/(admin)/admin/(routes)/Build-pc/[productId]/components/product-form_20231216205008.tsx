...
import { CPUSupport, Category, Image, Product } from "@prisma/client"
...
import { motherboardata } from "../../../Compatibilty-profile/[productId]/components/product-form";
...
const formSchema = z.object({
  ...
  pcTemplate: z.object({ url: z.string() }).array().min(1),
});
...
export const ProductForm: React.FC<ProductFormProps> = ({
  ...
  const onSubmit = async (data: ProductFormValues) => {
    ...
    try {
     ...
     if (initialData) {
       await axios.patch(`/api/prozzducts/${params.productId}`, data);
     } else {
       await axios.post(`/api/prozzducts`, data);
     }
     router.refresh();
  ...
  const [ramSlot,setramslot]=useState<RamSlots[]>(initialData?.RAMs.map((ee)=>({
    ...
    <>
      ...
      <Form {...form}>
        <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-8 w-full">
          ...
          />
          <Pctemplate initialData={null} motherboards={motherboards} cpus={cpus} gpus={gpus} rams={rams} diks={diks} powersupplies={powersupplies} cases={cases} motherboardData={[]} cpuData={[]}  />
          <Button onClick={()=>{

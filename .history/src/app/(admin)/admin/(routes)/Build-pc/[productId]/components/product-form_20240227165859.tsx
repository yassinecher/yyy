  ...
  const onSubmit = async (data: ProductFormValues) => {
    ...
    try {
      ...
      if(defaultMB.length==0){
        toast.error('set defaultMB ');
        return
      }
      if(defaultCPU.length==0){
        toast.error('set defaultCPU ');
        return
      }
      if(defaultGPU.length==0){
        toast.error('set defaultGPU ');
        return
      }
      if(defaultPR.length==0){
        toast.error('set defaultPR ');
        return
      }
      if(defaultCASE.length==0){
        toast.error('set defaultCASE ');
        return
      }

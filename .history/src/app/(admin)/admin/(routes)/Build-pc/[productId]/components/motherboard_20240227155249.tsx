...
const ProductCOl: React.FC<motherboardpar> = ({ init, colname, motherboards, selectionStaue, setMotherBoardId, onSlectionChange, updateList, MotherBoardId, lastMotherBoardId, setLastMotherBoardId }) => {
  ...
  useEffect(() => {
    if (colname == "mb" && setMotherBoardId && setLastMotherBoardId && motherboardList.length > 0) {
      setLastMotherBoardId(MotherBoardId)
              ...
              <div className='w-full pb-3 group '>
                {
                  item.images.length > 0 ? <>
                   ...
                   {item.price} TND <br />
                 {MotherBoardId!= item.id?<>
                  <Button className='w-full' onClick={()=>setMotherBoardId(item.id)}> Set Default</Button>
                 </>:<>

...
import { Category, Image, Product, Manufacturer, RamSlots, MotherboardChipset, CPUSupport, Guarantee, MotherboardFormat, Motherboard, Field, Laptop, LapRefreshRate, LapCamera, LapSound, Lapnetwork, Lapmemory, LapHardisk, LapScreenType, LapScreenSize, LapGraphiccardRef, LapGraphiccard, LapProcesseurRe, LapProcesseur, LapSystem, keyboard, keyboarButtonsNumber, keyboarFormat, keyboarTouchType, keyboarbrand, Headset, HeadsetModel, HeadsetSonSurround, HeadsetInterfaceAvecOrdinateur, RgbType, Sonsurround, Hautparleur, Chaisegaming } from "@prisma/client"
...
export const ProductForm: React.FC<ProductFormProps> = ({
  initialData,
  categories, RgbType,manufacturers
}) => {
  ...
  const defaultValues = initialData  && initialData.Chaisegaming ? {
    ...initialData,
    price: parseFloat(String(initialData?.price)),
    dicountPrice: parseFloat(String(initialData?.dicountPrice)),
    stock: parseFloat(String(initialData?.stock)),
    RgbTypeId:initialData.Chaisegaming[0].RgbTypeId??'',
    manufacturerId:initialData.Chaisegaming[0].manufacturerId??'',
...
console.log(initialData)
  ...
  const onSubmit = async (data: ProductFormValues) => {
    ...
    try {
      ...
      if (initialData) {
        await axios.patch(`/api/peripherique/Chaisegaming/component/${params.productId}`, data);
      } else {
        await axios.post(`/api/peripherique/Chaisegaming/component`, data);
      }
      router.refresh();
      router.push(`/admin/Chaisegaming`);

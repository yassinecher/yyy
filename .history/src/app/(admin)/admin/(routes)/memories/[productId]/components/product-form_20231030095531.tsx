...
import { Category, Image, Product, Manufacturer, RamSlots, MotherboardChipset, CPUSupport, Guarantee, MotherboardFormat, MemoryMarque, MemoryNumber, MemoryType, MemoryFrequency } from "@prisma/client"
...
export const ProductForm: React.FC<ProductFormProps> = ({
  ...
  const onSubmit = async (data: ProductFormValues) => {
    ...
    try {
      setLoading(true);
      if (initialData) {
        await axios.patch(`/api/memory/${params.productId}`, data);
      } else {
        await axios.post(`/api/memory`, data);
      }
      router.refresh();
...
<PopFormModal label={"Memory marque"} 
              ...
              formLab="marqueId"
              formCControlName="marqueId"
              IsNumber={false}
              />

         
<PopFormModal label={"Memory size"} 
              form1={form} 
              loading={loading} 
              setLoading={setLoading} 
              data={...memoryNumber}
              fieldaAfficher="number"
              url="/api/motherboard/MemoryNumber"
              formLab="chipsetId"
          ...
          <PopFormModal label={"Memory type"} 
              ...
              data={...memoryType}
              fieldaAfficher="name"
              url="/api/motherboard/MemoryType"
          ...
          <PopFormModal label={"Memory frequency"} 
              ...
              fieldaAfficher="name"
              url="/api/motherboard/MemoryFrequency"
              formLab="formatId"
              formCControlName="formatId"
              IsNumber={false}
              />

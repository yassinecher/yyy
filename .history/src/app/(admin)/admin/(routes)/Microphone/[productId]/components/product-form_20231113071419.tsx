...
import { Category, Image, Product, Manufacturer, RamSlots, MotherboardChipset, CPUSupport, Guarantee, MotherboardFormat, Motherboard, Field, Laptop, LapRefreshRate, LapCamera, LapSound, Lapnetwork, Lapmemory, LapHardisk, LapScreenType, LapScreenSize, LapGraphiccardRef, LapGraphiccard, LapProcesseurRe, LapProcesseur, LapSystem, keyboard, keyboarButtonsNumber, keyboarFormat, keyboarTouchType, keyboarbrand, Mic, MicInterfaceAvecOrdinateur, MicSonSurround, MicModel } from "@prisma/client"
...
interface ProductFormProps {
  ...
  categories: Category[];
  MicModel:MicModel[];
  MicInterfaceAvecOrdinateur:MicInterfaceAvecOrdinateur[]
  MicSonSurround:MicSonSurround[]
...
export const ProductForm: React.FC<ProductFormProps> = ({
  initialData,
  categories, MicModel, MicInterfaceAvecOrdinateur, MicSonSurround,manufacturers
}) => {
  ...
  const defaultValues = initialData  && initialData.Mic ? {
    ...initialData,
    price: parseFloat(String(initialData?.price)),
    dicountPrice: parseFloat(String(initialData?.dicountPrice)),
    stock: parseFloat(String(initialData?.stock)),
    micModelId:initialData.Mic[0].micModelId??'',
    micInterfaceAvecOrdinateurId:initialData.Mic[0].micInterfaceAvecOrdinateurId??'',
    micSonSurroundId:initialData.Mic[0].micSonSurroundId??'',
    wireless:initialData.Mic[0].wireless??false,
    rgb:initialData.Mic[0].rgb??false,
    manufacturerId:initialData.Mic[0].manufacturerId??'',
...
console.log(initialData)
  ...
  const onSubmit = async (data: ProductFormValues) => {
    ...
    try {
      ...
      if (initialData) {
        await axios.patch(`/api/Mic/component/${params.productId}`, data);
      } else {
        await axios.post(`/api/Mic/component`, data);
      }
      router.refresh();
...
<PopFormModal label={"Interface Avec Ordinateur"} 
              ...
              formCControlName="MicInterfaceAvecOrdinateur"
              IsNumber={false}
              />

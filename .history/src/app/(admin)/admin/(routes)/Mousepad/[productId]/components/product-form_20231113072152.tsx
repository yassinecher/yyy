...
import { Category, Image, Product, Manufacturer, RamSlots, MotherboardChipset, CPUSupport, Guarantee, MotherboardFormat, Motherboard, Field, Laptop, LapRefreshRate, LapCamera, LapSound, Lapnetwork, Lapmemory, LapHardisk, LapScreenType, LapScreenSize, LapGraphiccardRef, LapGraphiccard, LapProcesseurRe, LapProcesseur, LapSystem, keyboard, keyboarButtonsNumber, keyboarFormat, keyboarTouchType, keyboarbrand, Mousepad } from "@prisma/client"
...
interface ProductFormProps {
  ...
  categories: Category[];
  MousepadModel:keyboarbrand[];
  MousepadSize:keyboarFormat[]
...
export const ProductForm: React.FC<ProductFormProps> = ({
  initialData,
  categories, MousepadModel, MousepadSize,manufacturers
}) => {
  ...
  const defaultValues = initialData  && initialData.Mousepad ? {
    ...initialData,
    price: parseFloat(String(initialData?.price)),
    dicountPrice: parseFloat(String(initialData?.dicountPrice)),
    stock: parseFloat(String(initialData?.stock)),
    mousepadModelId:initialData.Mousepad[0].mousepadModelId??'',
    mousepadSizeId:initialData.Mousepad[0].mousepadSizeId??'',

    rgb:initialData.Mousepad[0].rgb??false,
    manufacturerId:initialData.Mousepad[0].manufacturerId??'',
    ...
    <>
      ...
      <Form {...form}>
        ...
        <FormField
          ...
          <PopFormModal label={"Mousepad Model"} 
              form1={form} 
              loading={loading} 
              setLoading={setLoading} 
              data={...MousepadModel}
              fieldaAfficher="name"
              url="/api/Mousepad/keyboarbrand"
              formLab="mousepadModelId"
              formCControlName="mousepadModel"
              IsNumber={false}
              />

<PopFormModal label={"Size"} 
              form1={form} 
              loading={loading} 
              setLoading={setLoading} 
              data={...MousepadSize}

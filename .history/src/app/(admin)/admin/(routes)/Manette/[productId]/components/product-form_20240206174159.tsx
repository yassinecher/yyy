...
import { Category, Image, Product, Manufacturer, RamSlots, MotherboardChipset, CPUSupport, Guarantee, MotherboardFormat, Motherboard, Field, Laptop, LapRefreshRate, LapCamera, LapSound, Lapnetwork, Lapmemory, LapHardisk, LapScreenType, LapScreenSize, LapGraphiccardRef, LapGraphiccard, LapProcesseurRe, LapProcesseur, LapSystem, keyboard, keyboarButtonsNumber, keyboarFormat, keyboarTouchType, keyboarbrand, Headset, HeadsetModel, HeadsetSonSurround, HeadsetInterfaceAvecOrdinateur, RgbType, Sonsurround, Hautparleur, Manette, Connectivite } from "@prisma/client"
...
export const ProductForm: React.FC<ProductFormProps> = ({
  initialData,
  categories, RgbType, Connectivite,manufacturers
}) => {
  ...
  const defaultValues = initialData  && initialData.Manette ? {
    ...initialData,
    price: parseFloat(String(initialData?.price)),
    dicountPrice: parseFloat(String(initialData?.dicountPrice)),
    stock: parseFloat(String(initialData?.stock)),
    Connectiviteid:initialData.Manette[0].Connectiviteid??'',
    RgbTypeId:initialData.Manette[0].RgbTypeId??'',
    manufacturerId:initialData.Manette[0].manufacturerId??'',
...
<PopFormModal label={"Connectivité"} 
              form1={form} 
              loading={loading} 
              setLoading={setLoading} 
              data={Connectivite}
              fieldaAfficher="name"
              url="/api/peripherique/Manette/Connectivite"
              formLab="Connectiviteid"
              formCControlName="Connectivite"
              IsNumber={false}
              />

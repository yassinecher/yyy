...
import { Category, Image, Product, Manufacturer, RamSlots, MotherboardChipset, CPUSupport, Guarantee, MotherboardFormat, Motherboard, Field, Laptop, LapRefreshRate, LapCamera, LapSound, Lapnetwork, Lapmemory, LapHardisk, LapScreenType, LapScreenSize, LapGraphiccardRef, LapGraphiccard, LapProcesseurRe, LapProcesseur, LapSystem, keyboard, keyboarButtonsNumber, keyboarFormat, keyboarTouchType, keyboarbrand, Headset, HeadsetModel, HeadsetSonSurround, HeadsetInterfaceAvecOrdinateur, Camera, CameraType } from "@prisma/client"
...
export const ProductForm: React.FC<ProductFormProps> = ({
  initialData,
  categories, CameraType,manufacturers
}) => {
  ...
  const defaultValues = initialData  && initialData.Camera ? {
    ...initialData,
    price: parseFloat(String(initialData?.price)),
    dicountPrice: parseFloat(String(initialData?.dicountPrice)),
    stock: parseFloat(String(initialData?.stock)),
    CameraTypeId:initialData.Camera[0].mousepadModelId??'',
   
    manufacturerId:initialData.Camera[0].manufacturerId??'',
...
console.log(initialData)
  ...
  const onSubmit = async (data: ProductFormValues) => {
    ...
    try {
      ...
      if (initialData) {
        await axios.patch(`/api/peripherique/Camera/component/${params.productId}`, data);
      } else {
        await axios.post(`/api/peripherique/Camera/component`, data);
      }
      router.refresh();
...
<PopFormModal label={"Camera Type"} 
              form1={form} 
              loading={loading} 
              setLoading={setLoading} 
              data={CameraType}
              fieldaAfficher="name"
              url="/api/peripherique/Camera/CameraType"
              formLab="CameraTypeId"
              formCControlName="CameraTypeId"
              IsNumber={false}
              />

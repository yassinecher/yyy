...
type motherboardpar = {
  ...
  lastMotherBoardId: String
  MotherBoardId: String[]
  setLastMotherBoardId?: (value: String) => void;
  setMotherBoardId: (ramid :String,key:number) => void;
  ListKey:number
}

const ProductCOlRam: React.FC<motherboardpar> = ({ init,ListKey, colname, motherboards, selectionStaue, setMotherBoardId, onSlectionChange, updateList, MotherBoardId, lastMotherBoardId, setLastMotherBoardId }) => {
  const previewModal = usePreviewModal();
  const [motherboardList, setOtherboardListt] = useState<ProdCol[]>(init);
  ...
  useEffect(() => {
    if ( motherboardList.length > 0) {
      setMotherBoardId(motherboardList[0].id,ListKey )
    }else{
      setMotherBoardId("",ListKey )
    }
  }, [motherboardList])
  ...
  useEffect(() => {
    ...
    router.refresh()
  }, [selectionStaue, colname, router, MotherBoardId])
  ...
  const removeItem = (itemToRemove: string) => {
    ...
    if (colname == "mb" && updatedItems.length === 0) {
      if (setLastMotherBoardId)
        setLastMotherBoardId('')
      onSlectionChange({ ...selectionStaue, motherboardIsSelected: false })
      router.refresh()
    }
              ...
              <div className='w-full pb-3 group '>
                {
                  item.images.length > 0 ? <>
                   ...
                   {item.name} <br />
                   {item.price} TND <br />
                 {MotherBoardId[ListKey]!= item.id?<>
                  <button type='button' className='w-auto bg-green-500 m-2  text-white p-2 rounded-sm' onClick={()=>setMotherBoardId(item.id,ListKey)}> Set Default</button>
                 </>:<>
                 <button   type='button'  className='w-auto bg-red-500 m-2 text-white  p-2 rounded-sm' >Default</button>
